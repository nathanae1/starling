// Builds the canonical protocol vector map pinned in `index.json`.
//
// Shared by `vectors_gen.dart` (the diff-asserting regeneration test) so
// there is exactly ONE definition of every pinned surface. Values must be
// JSON-encodable (hex strings, ints, bools, lists) — never raw bytes.
...
/// Fixed timestamp for the request-bound owner-signature vectors: the
/// bound digest covers `u64_be(ts)`, so the vector pins one deterministic
/// value (the relay's live skew check is exercised in its own tests).
const int kVectorSignedTs = 1700000100;

Future<Map<String, dynamic>> buildProtocolVectors() async {
  ...
  // Owner write auth, legacy scheme: Ed25519 over blake2b256(body).
  // Retained one release for self-hosted relays that predate M2.
  final ownerSigDigest = crypto.blake2b256(pushBatchCbor);
  final ownerSig = crypto.sign(kp.secretKey, ownerSigDigest);

  // A PushItem with a non-"event" type (F4 preserve-and-forward): the
  // relay must store and echo the unknown type, and a decoder must default
  // a missing type to "event".
  final typedPushBatchCbor = Uint8List.fromList(
    cbor.encode(<String, dynamic>{
      'items': [
        <String, dynamic>{
          'id': 'future-item-1',
          'payload': Uint8List.fromList(List.filled(8, 0xEE)),
          'type': 'future-thing',
        },
      ],
    }),
  ...
  // POST /events/delete + /media/delete (Dart-produced, Phase 3), signed
  // with the request-bound scheme (M2) at a pinned timestamp.
  final deleteEventsIds = [eventId, 'evt-already-gone'];
  final deleteEventsCbor = Uint8List.fromList(
    cbor.encode(<String, dynamic>{'ids': deleteEventsIds}),
  );
  final deleteEventsDigest = ownerRequestDigest(
    crypto: crypto,
    method: 'POST',
    path: '/events/delete',
    unixTs: kVectorSignedTs,
    body: deleteEventsCbor,
  );
  final deleteEventsSig = crypto.sign(kp.secretKey, deleteEventsDigest);

  final deleteMediaHashes = ['aa' * 32, 'bb' * 32];
  final deleteMediaCbor = Uint8List.fromList(
    cbor.encode(<String, dynamic>{'hashes': deleteMediaHashes}),
  );

  // DeleteReceipt (Rust-produced).
  final deleteReceiptCbor = Uint8List.fromList(
    cbor.encode(<String, dynamic>{'deleted': 1, 'missing': 1}),
  );
  ...
  final manifestPageCbor = Uint8List.fromList(
    cbor.encode(<String, dynamic>{
      'pubkey': crockfordBase32Encode(kp.publicKey),
      'events': [
        <String, dynamic>{'id': eventId, 'created_at': 1700000000},
      ],
      'has_older': true,
    }),
  );
  ...
  return {
    '_comment':
        'Canonical protocol test vectors for Starling crypto and the '
        'Dart<->Rust relay wire. Regenerate via STARLING_WRITE_VECTORS=1 '
        'flutter test test/vectors/vectors_gen.dart (which otherwise '
        'diff-asserts this file). Dart tests assert against them '
        '(vectors_test.dart, http_vectors_test.dart); the Rust relay '
        'asserts relay_wire in relay/crates/starling-wire/tests/'
        'wire_vectors.rs and http in relay/crates/starling-relay-http/'
        'tests/http_vectors.rs.',
    // F3: BOTH `kStarlingProtocolVersion` (Dart) and `PROTOCOL_VERSION`
    // (Rust) assert against this single entry, so a bump on one side
    // fails the other side's tests.
    'protocol_version': kStarlingProtocolVersion,
    'keypair': {
      'seed_hex': hex(seed),
    ...
    'relay_wire': {
      ...
      'push_batch_typed': {
        '_comment':
            'F4 preserve-and-forward: a non-"event" item type must '
            'round-trip the relay tier unchanged.',
        'item_id': 'future-item-1',
        'item_type': 'future-thing',
        'payload_hex': 'ee' * 8,
        'cbor_hex': hex(typedPushBatchCbor),
      },
      'push_receipt': {
        'accepted': 1,
        'rejected': 0,
      ...
      'delete_events_request': {
        'ids': deleteEventsIds,
        'cbor_hex': hex(deleteEventsCbor),
        'signed_ts': kVectorSignedTs,
        'signed_path': '/events/delete',
        'owner_req_digest_hex': hex(deleteEventsDigest),
        'owner_sig_hex': hex(deleteEventsSig),
      },
      'delete_media_request': {
        'hashes': deleteMediaHashes,
        'cbor_hex': hex(deleteMediaCbor),
      },
      'delete_receipt': {
        'deleted': 1,
        'missing': 1,
        'cbor_hex': hex(deleteReceiptCbor),
      },
    ...
    'http': {
      '_comment':
          'HTTP-layer contract per relay endpoint: method, path, query '
          'param names, auth requirement, and success status. Client side '
          'asserted by test/vectors/http_vectors_test.dart (production '
          'RelayPushService/RelayPushCoordinator requests), server side by '
          'relay/crates/starling-relay-http/tests/http_vectors.rs (axum '
          'oneshot, including query-param EFFECT so a renamed param fails).',
      'auth_headers': ['x-starling-pubkey', 'x-starling-ts', 'x-starling-sig'],
      'endpoints': {
        'status': {
          'method': 'GET',
          'path': '/status',
